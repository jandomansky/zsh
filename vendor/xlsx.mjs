...
function SSF_init_table(t/*:any*/) {
	...
	t[48]= '##0.0E+0';
	t[49]= '@';
	t[56]= '"ä¸ŠĺŤ/ä¸‹ĺŤ "hh"ć™‚"mm"ĺ†"ss"ç§’ "';
	return t;
}
...
function fmt_is_date(fmt/*:string*/)/*:boolean*/ {
	...
	while(i < fmt.length) {
		switch((c = fmt.charAt(i))) {
			...
			case 'A': case 'a': case 'ä¸Š':
				if(fmt.substr(i, 3).toUpperCase() === "A/P") return true;
				if(fmt.substr(i, 5).toUpperCase() === "AM/PM") return true;
				if(fmt.substr(i, 5).toUpperCase() === "ä¸ŠĺŤ/ä¸‹ĺŤ") return true;
				++i; break;
			case '[':
				o = c;
	...
	while(i < fmt.length) {
		switch((c = fmt.charAt(i))) {
			...
			case 'A': case 'a': case 'ä¸Š':
				...
				else if(fmt.substr(i,5).toUpperCase() === "ä¸ŠĺŤ/ä¸‹ĺŤ") { if(dt!=null) q.v = dt.H >= 12 ? "ä¸‹ĺŤ" : "ä¸ŠĺŤ"; q.t = 'T'; i+=5; hr='h'; }
				else { q.t = "t"; ++i; }
...
function cc2str(arr/*:Array<number>*/, debomit)/*:string*/ {
	...
	if(typeof TextDecoder !== "undefined") try {
		...
		return new TextDecoder("latin1").decode(arr).replace(/[â‚¬â€šĆ’â€žâ€¦â€ â€ˇË†â€°Ĺ â€ąĹ’Ĺ˝â€â€™â€śâ€ťâ€˘â€“â€”Ëśâ„˘Ĺˇâ€şĹ“ĹľĹ¸]/g, function(c) { return rev[c] || c; });
	} catch(e) {}
...
function load_props_pairs(HP/*:string|Array<Array<any>>*/, TOP, props, opts) {
	...
	if(parts.length > 0) for(var i = 0; i !== v.length; i += 2) {
		...
		switch(v[i].v) {
			...
			case "ĺ·Ąä˝śčˇ¨":
			case "Đ›Đ¸ŃŃ‚Ń‹":
			case "ŘŁŮŘ±Ř§Ů‚ Ř§Ů„ŘąŮ…Ů„":
			case "ăŻăĽă‚Żă‚·ăĽă":
			case "×’×ś×™×•× ×•×Ş ×˘×‘×•×“×”":
			case "ArbeitsblĂ¤tter":
			case "Ă‡alÄ±Ĺźma SayfalarÄ±":
			case "Feuilles de calcul":
			case "Fogli di lavoro":
			case "Folhas de cĂˇlculo":
			case "Planilhas":
			case "Regneark":
			case "Hojas de cĂˇlculo":
			case "Werkbladen":
...
var SYLK = /*#__PURE__*/(function() {
	...
	var sylk_escapes = ({
		AA:'Ă€', BA:'Ă', CA:'Ă‚', DA:195, HA:'Ă„', JA:197,
		AE:'Ă', BE:'Ă‰', CE:'ĂŠ',         HE:'Ă‹',
		AI:'ĂŚ', BI:'ĂŤ', CI:'ĂŽ',         HI:'ĂŹ',
		AO:'Ă’', BO:'Ă“', CO:'Ă”', DO:213, HO:'Ă–',
		AU:'Ă™', BU:'Ăš', CU:'Ă›',         HU:'Ăś',
		Aa:'Ă ', Ba:'Ăˇ', Ca:'Ă˘', Da:227, Ha:'Ă¤', Ja:229,
		Ae:'Ă¨', Be:'Ă©', Ce:'ĂŞ',         He:'Ă«',
		Ai:'Ă¬', Bi:'Ă­', Ci:'Ă®',         Hi:'ĂŻ',
		Ao:'Ă˛', Bo:'Ăł', Co:'Ă´', Do:245, Ho:'Ă¶',
		Au:'Ăą', Bu:'Ăş', Cu:'Ă»',         Hu:'ĂĽ',
		KC:'Ă‡', Kc:'Ă§', q:'Ă¦',  z:'Ĺ“',  a:'Ă†',  j:'Ĺ’',
...
function write_theme(Themes, opts)/*:string*/ {
	...
	o[o.length] =     '<a:font script="Jpan" typeface="ďĽ­ďĽł ďĽ°ă‚´ă‚·ăă‚Ż"/>';
	o[o.length] =     '<a:font script="Hang" typeface="ë§‘ěť€ ęł ë”•"/>';
	o[o.length] =     '<a:font script="Hans" typeface="ĺ®‹ä˝“"/>';
	o[o.length] =     '<a:font script="Hant" typeface="ć–°ç´°ćŽé«”"/>';
...
function parse_content_xml(d/*:string*/, _opts, _nfm)/*:Workbook*/ {
		...
		while((Rn = xlmlregex.exec(str))) switch((Rn[3]=Rn[3].replace(/_[\s\S]*$/,""))) {
			...
			case 'table': case 'ĺ·Ąä˝śčˇ¨': // 9.1.2 <table:table>
				if(Rn[1]==='/') {
					...
					sheetag.name = sheetag['ĺŤç§°'] || sheetag.name;
			...
			case 'table-row-group': // 9.1.9 <table:table-row-group>
				...
				break;
			case 'table-row': case 'čˇŚ': // 9.1.3 <table:table-row>
				if(Rn[1] === '/') { R+=rowpeat; rowpeat = 1; break; }
				rowtag = parsexmltag(Rn[0], false);
				if(rowtag['čˇŚĺŹ·']) R = rowtag['čˇŚĺŹ·'] - 1; else if(R == -1) R = 0;
			...
			case 'table-cell': case 'ć•°ćŤ®':
				if(Rn[0].charAt(Rn[0].length-2) === '/') {
					...
					if((ctag['ć•°ćŤ®ç±»ĺž‹'] || ctag['value-type']) == "string") {
					...
					q = ({t:ctag['ć•°ćŤ®ç±»ĺž‹'] || ctag['value-type'], v:null/*:: , z:null, w:"",c:[]*/}/*:any*/);
					if(ctag["style-name"] && styles[ctag["style-name"]]) q.z = styles[ctag["style-name"]];
					if(opts.cellFormula) {
					...
					switch(q.t) {
						...
						case 'number': q.t = 'n'; q.v = parseFloat(ctag['ć•°ćŤ®ć•°ĺ€Ľ']);
							break;
						default:
			...
			case 'document-content': case 'ç”µĺ­čˇ¨ć Ľć–‡ćˇŁ': // 3.1.3.2 <office:document-content>
			...
			/* ignore state */
			case 'meta': case 'ĺ…ć•°ćŤ®': // TODO: <office:meta> <uof:ĺ…ć•°ćŤ®> FODS/UOF
			...
			case 'body': case 'ç”µĺ­čˇ¨ć Ľ': break; // 3.3 16.9.6 19.726.3
			...
			case 'p': case 'ć–‡ćś¬ä¸˛': // 5.1.3 <text:p>
				if(['master-styles'].indexOf(state[state.length-1][0]) > -1) break;
				if(Rn[1]==='/' && (!ctag || !ctag['string-value'])) {
					var ptp = parse_text_p(str.slice(textpidx,Rn.index), textptag);
					textp = (textp.length > 0 ? textp + "\n" : "") + ptp[0];
				} else if(Rn[0].slice(-2) == "/>") {
					/* TODO: is self-closing ć–‡ćś¬ä¸˛ valid? */
					textp += "\n";
			...
			case 'title': case 'ć ‡é˘': break; // <*:title> OR <uof:ć ‡é˘>
